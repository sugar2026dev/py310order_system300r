...
// 创建axios实例，自动处理API_BASE_URL
const apiClient = axios.create({
  baseURL: API_BASE_URL,
  xsrfCookieName: 'csrftoken',
  xsrfHeaderName: 'X-CSRFToken',
  withCredentials: true,
  timeout: APP_CONFIG.TIMEOUTS ? APP_CONFIG.TIMEOUTS.API_REQUEST : 10000
});
  ...
  setup() {
    ...
    const buildImageUrl = (order) => {
      ...
      if (order.img_filename && order.img_filename.trim() !== '') {
        ...
        directories.forEach(dir => {
          possibleUrls.push({ url: dir + filename, source: `img_filename + ${dir}`, priority: 3 });
        });
      }
    ...
    const deleteOrder = async (order) => {
      try {
        ...
        const response = await apiClient.post('/orders/batch-delete/', {
          order_ids: [order.id]
        });
    ...
    const handleImageUpload = async (file) => {
      ...
      try {
        ...
        const uploadResponse = await apiClient.post('/orders/ocr-for-form/', formData, {
        ...
        if (uploadResponse.data.code === 200) {
          ...
          try {
            const response = await apiClient.get(`/orders/check/${orderCode}/`, {
              timeout: 10000
            });
            ...
            if (response.data.code === 200 && response.data.data.exists) {
              ...
              if (ocrData.img_path && ocrData.img_path.trim() !== '') {
                ...
                if (!serverImageUrl.startsWith('http')) {
                  serverImageUrl = serverImageUrl.startsWith('/') ? serverImageUrl : '/' + serverImageUrl;
                }
    ...
    const submitCustomOrderForm = async () => {
      ...
      try {
        ...
        if (!isEdit.value) {
          try {
            const checkResponse = await apiClient.get(`/orders/check/${orderData.order_code}/`);
        ...
        if (isEdit.value) {
          response = await apiClient.post('/orders/update/', orderData);
        } else {
          response = await apiClient.post('/orders/add/', orderData);
        }
